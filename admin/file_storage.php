<?php
session_start();
ob_start();

include_once '../connection/dbconnection.php';
include_once '../helpers/module.php';
include_once '../resources/links/require.php';

date_default_timezone_set("Asia/Kuala_Lumpur");

if(isset($_POST['upload']))
{	

	$date_uploaded = date('d-m-Y');

	$name = strip_tags($_POST['name']);
	$name = mysqli_real_escape_string($conn, $name);


	$fileCount = count($_FILES['file']['name']);
		for($i=0; $i<$fileCount; $i++){
		
			$fileName = $_FILES['file']['name'][$i];
			$target_dir = "../upload/file_storage/";


			$check_storagename= check_storagename($fileName);
			$rowcount=mysqli_num_rows($check_filename);

			if($rowcount == 0)

			{	
				insert_storage($date_uploaded, $name, $fileName);
				move_uploaded_file($_FILES['file']['tmp_name'][$i],$target_dir.$fileName);
				header('location: current_file_storage.php');
			}

			else
			{
				?>
					<script>
						alert("File name has been taken, please change file name and try again.");
					</script>
				<?php
			}		
		}
}

?>


<!DOCTYPE html>
<html lang="en">
	<head>
		<?php include_once('../head/head.php'); ?>
		<title>File Storage</title>

	</head>

	<body>

		<?php
		include_once('../navigation/top_nav.php');
		include_once('../navigation/side_nav.php');
		?>

		<div class="content">
			<div class="resume-top-left-button">
				<!-- <input type="button" name="all-resume-button" value="All" id="resume-button-all" class="resume-button-all active-buttons" onclick="location.href='file_storage.php'"> -->
				<input type="button" name="current-resume-button" value="Current" id="resume-button-current" class="resume-button-current" onclick="location.href='current_file_storage.php'">
				<input type="button" name="archive-resume-button" value="Archive" id="resume-button-archive" class="resume-button-archive" onclick="location.href='archive_file_storage.php'">
			</div>

            <div class="file-display">
				<div id="data-table" class="table-display file-width">
					<table class="table-content">
						<thead class="header">
							<th class="med-col">File Name</th>
							<th class="big-col">File</th>
							<th class="med-col">Date Uploaded</th>
							<th class="small-col">Status</th>
						</thead>
                            <tbody>
                            <?php 
                                $get_AllStorage = get_AllStorage();
                            
                                while($row = mysqli_fetch_array($get_AllStorage))
                                {
                            ?>
                                    <tr class='tbl-link'>
                                        <td onclick="location.href='view_storage.php?storageid=<?php echo $row['storage_id']; ?>'"><?php echo $row['name']; ?></td>
                                        <td onclick="location.href='view_storage.php?storageid=<?php echo $row['storage_id']; ?>'"><?php echo $row['file']; ?></td>
										<td onclick="location.href='view_storage.php?storageid=<?php echo $row['storage_id']; ?>'"><?php echo $row['date_uploaded']; ?></td>
                                        <td onclick="location.href='view_storage.php?storageid=<?php echo $row['storage_id']; ?>'"><?php echo $row['status']; ?></td>
										
                                        <!-- <td>
                                            <?php
                                                $confirmation = "Are you sure about archiving the selected file?";
                                            ?>
                                            <a href='archive.php?storageid=<?php echo $row['storage_id'];?>&status=Archive' onclick="return confirm('<?php echo $confirmation; ?>')">
                                                <img src="../resources/images/cardboard-box.png" class='smicon'>
                                            </a>
                                        </td> -->
                                    </tr>
                            <?php
                                }	
                            ?>
                        </tbody>
					</table>
				</div>
			</div>

			<div class="fs-button">
				<div class="s-button-one">
					<input type="submit" value="Add Files" class="update-chart" onclick="openFile()">
				</div>
			</div>
			

			<div class="fs-popup" id="s-form">
				<form method="POST" enctype="multipart/form-data">
					
					<div class="fs-popup-one">
						<input type="text" name="name" id="file-name" placeholder="File Name" autocomplete="off" required>
					</div>

					<div class="fs-popup-three">
						<input type="file" name="file[]" class="custom-fs-input" id="custom-fs-input" accept=".pdf,.doc,.docx,.xlsx,.csv,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document" required>
					</div>

					<div class="fs-popup-two">
						<input type="submit" name="upload" value="Upload" id="submit-fs" class="submit-fs">
						<input type="button" name="submit" value="Back" id="back-fs" class="back-fs" onclick="closeFile()">
					</div>
				</form>
			</div>
		</div>
		<script src="../dependencies/scripts/storage.js"></script>
    </body>
</html>
